...
- **Don't reorder `<script>` tags casually.** `canvas.js` must come before any code using `tileSize`/`ctx` at call time (currently it's 4th). Order: config → state → world → canvas → ai → render → save → ui → main.
